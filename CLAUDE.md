...
│   ├── ldpc_decoder.h                  GPU LDPC decoder interface (batch ldpc_decode) + cb configuration
│   ├── ldpc_decoder_cuda_helpers.h     H2D/D2H copy, memset, zero helpers (templated)
│   ├── cuda_stream.h                   RAII CUDA stream + completion callback support
...
└── phy/upper/
    ...
    └── channel_processors/pusch/                   PUSCH integration layer
        ...
        ├── pusch_codeblock_cuda_decoder.h/.cpp     Single codeblock: CUDA LDPC decode + CRC
...
softbits → pusch_decoder_cuda_impl (state machine)
  │
  ├─ new_data()          – allocate buffers, set config, compute segmentation
  ...
  └─ on_end_softbits()   – segment remaining codeblocks, wait for all decodes → join & notify
         ...
         └─ pusch_codeblock_cuda_decoder (per codeblock)
              └─ decode()      – GPU LDPC decode (rate dematch embedded in kernel)
                    │         → soft-bit loading → LDPC iterations → hard decision → CRC check
                    │
                    └─ cuda_ldpc_decode kernel (ldpc_decoder_impl.cu)
...
| `cuda_ldpc_decoder_asynchronous_backend` | Pool of 128 CUDA streams; batches dispatched on fill or `last_codeblock` flag (no timeout fallback) |
...
| `pusch_codeblock_cuda_decoder` | Wraps decoder + CRC for one codeblock (rate dematching done in GPU kernel) |
...
- Batches are dispatched immediately when full (32 codeblocks) or when the `last_codeblock` flag is set — no timeout fallback.
...
- This plugin depends on the parent OCUDU project's headers — do not modify parent paths
